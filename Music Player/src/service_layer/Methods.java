 ...
 * @author Harsh Sharma
 * 
 *         A class to instantiate and set all GUI components in position and in
 *         sync with their subsequent operations
 */
public class Methods extends More_Methods implements ActionListener {
		...
		volume_Slider.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent me) {
				...
				try {
					...
					JOptionPane.showMessageDialog(f1, e.getMessage());
				}
			}
		});
			...
			try {
				...
				if (play_button_is_pressed % 2 != 0) {
					if(filepath.contains(".mp3") || 
								!filepath.contains(".mp3") && mp3.getPlayList().size() > 0) {
						eventForPlayLabel();
						play.setIcon(ic11);
					}
				}
				...
				JOptionPane.showMessageDialog(f1, "No suitable media file selected, kindly retry !");
			}
		}
...
//		}
		...
		// Connect To Server frame and components settings are ending here //
